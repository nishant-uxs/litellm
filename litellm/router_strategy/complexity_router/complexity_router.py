...
def _extract_prior_user_turns(
    ...
    """Up to window_size human asks preceding current_ask, oldest first.

    The current ask is classified on its own, so including it would duplicate it and displace the
    oldest prior turn. Excluded by matching current_ask rather than by position, since `aclassify`
    takes `prompt` and `messages` separately and may classify other than the newest turn.
    """
...
class ComplexityRouter(CustomLogger):
    ...
    def _build_classifier_user_payload(
        ...
        The depth signal gates on `prior_turns` rather than on `messages` being non-empty: with the
        window at 0 nothing about the conversation may be sent, and on a single-turn request a depth
        line would report the size of the ask itself as history.
        """
        caller_prompt_block = (
